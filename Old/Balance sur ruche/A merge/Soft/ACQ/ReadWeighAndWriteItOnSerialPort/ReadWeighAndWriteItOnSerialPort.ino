...
#define TARVALUE 8413902
#define CALVALUE -20.61
...
void loop() {
  ...
  if (newDataReady) {
    if (millis() > (t + serialPrintInterval)) {
      raw = LoadCell.getData(); //getData type : float
	  raw/=100;//getData est exprimé en g : 25,9kg = 25900.00
      Serial.write(raw >> 8); 
      Serial.write(raw & 0x00FF);
      newDataReady = 0;
      t = millis();
    }
  }
